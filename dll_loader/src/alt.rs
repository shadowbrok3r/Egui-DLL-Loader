...
use winapi::um::winnt::{
    HANDLE as WINNT_HANDLE, MEM_COMMIT, MEM_RELEASE, MEM_RESERVE, PAGE_EXECUTE_READWRITE,
};
...
fn main() -> io::Result<()> {
    ...
    let mut file_contents = vec![0u8; filesize];
    ...
    // let source_image = get_loaded_image(buffer);
    // let source_headers = get_nt_headers(buffer);
