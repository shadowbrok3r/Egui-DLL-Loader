;; Simple calc.exe shellcode embedded in WASM
;; This is a minimal x64 shellcode that spawns calc.exe without complex API resolution
(module
  (memory 1)
  
  ;; x64 calc.exe shellcode - position independent, minimal dependencies
  (data (i32.const 0)
  ...
  ;; Function to get the size of our shellcode
  (func (export "get_wasm_mem_size") (result i32)
    i32.const 276  ;; Size of our calc.exe shellcode
  )
  
  ;; Function to read shellcode bytes at index
  (func (export "read_wasm_at_index") (param $index i32) (result i32)
    local.get $index
    i32.load8_u  ;; Load byte at index and return as i32
  )
)
